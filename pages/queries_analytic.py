...
"""
odi_batting_performance_df = pd.read_sql_query(query_odi_batting_performance, conn1)
...
"""
t20_batting_performance_df = pd.read_sql_query(query_t20_batting_performance, conn2)

combined_batting_performance_df = pd.concat([odi_batting_performance_df, t20_batting_performance_df])
...
batting_performance_with_names_df = pd.merge(combined_batting_performance_df, combined_player_names_df, on='player_id', how='left')

batting_performance_with_names_df['match_date'] = pd.to_datetime(batting_performance_with_names_df['match_date'])
batting_performance_with_names_df = batting_performance_with_names_df.sort_values(by=['player_id', 'match_date'])
last_10_performances = batting_performance_with_names_df.groupby('player_id').tail(10).copy()
...
def analyze_recent_form(df):
    ...
    consistency_score = 1 / (std_dev_runs + 1) # Add 1 to avoid division by zero

    # Categorize form (example logic, can be adjusted)
...
player_recent_form = last_10_performances.groupby('player_id').apply(analyze_recent_form).reset_index()
final_recent_form_df = pd.merge(player_recent_form, combined_player_names_df, on='player_id', how='left')
